...
export const addNewPost = (postData) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post("http://localhost:4000/api/v1/article/new", postData, config);
...
export const getPostsOfFollowing = (page = 1) => async (dispatch) => {
    try {
        ...
        setTimeout(async () => {
            ...
            const { data } = await axios.get(`http://localhost:4000/api/v1/article?page=${page}`);
...
export const likePost = (postId) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`http://localhost:4000/api/v1/article/${postId}`);
...
export const addComment = (postId, comment) => async (dispatch) => {
    try {
        ...
        const config = { header: { "Content-Type": "application/json" } }
        const { data } = await axios.post(`http://localhost:4000/api/v1/article/comment/${postId}`, { comment }, config);
...
export const savePost = (postId) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post(`http://localhost:4000/api/v1/article/${postId}`);
...
export const deletePost = (postId) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.delete(`http://localhost:4000/api/v1/article/${postId}`);
...
export const getPostDetails = (postId) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`http://localhost:4000/api/v1/article/detail/${postId}`);
